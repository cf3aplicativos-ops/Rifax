...
// Menú lateral azul marino, colapsable en escritorio y con cajón (drawer) en
// móvil. Ítem activo resaltado en amarillo (--rifax-accent). Es el mismo
// componente para el aplicativo del administrador, el portal del vendedor y el
// del cliente, para que todos luzcan igual.
...
export default function SideNav({
  nav, brand, subtitle, logoUrl, homeHref = "/", footer, children,
}: {
  nav: NavItem[];
  brand: string;
  subtitle?: string;
    ...
    <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
      {nav.map((n) => (
        <Link
          ...
          title={n.label}
          className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition ${activo(n) ? ACTIVE : IDLE} ${collapsed ? "lg:justify-center" : ""}`}
        >
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className={`absolute inset-y-0 left-0 flex w-64 flex-col ${SIDEBAR}`}>
            <div className="flex items-center justify-between border-b border-white/10 p-3">
              {Logo}
              <button onClick={() => setMobileOpen(false)} aria-label="Cerrar" className="rounded-lg p-1.5 text-slate-400 hover:bg-white/10"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12" /></svg></button>
            </div>
            {NavLinks}
