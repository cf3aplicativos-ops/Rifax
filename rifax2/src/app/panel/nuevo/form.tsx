...
export default function FormNuevoTenant({ sedesBasico, sedesCorporativo }: { sedesBasico: number; sedesCorporativo: number }) {
  ...
  const [usuariosIlim, setUsuariosIlim] = useState(true);
  const [maxSedes, setMaxSedes] = useState(String(sedesBasico));
  ...
  // Al elegir el plan se ajusta el número de sedes sugerido (configurable en
  // Facturación); los usuarios son ilimitados en ambos planes por defecto.
  function cambiarPlan(p: string) {
    setPlan(p);
    setSedesIlim(false);
    setUsuariosIlim(true);
    setMaxSedes(String(p === "corporativo" ? sedesCorporativo : sedesBasico));
  }
        ...
        <fieldset className="space-y-4 rounded-xl border border-slate-300 p-4 dark:border-slate-700">
          ...
          <div>
            ...
            <select id="plan" name="plan" value={plan} onChange={(e) => cambiarPlan(e.target.value)} className={campo}>
              <option value="basico">Básico ({sedesBasico} sede{sedesBasico === 1 ? "" : "s"} · usuarios ilimitados)</option>
              <option value="corporativo">Corporativo ({sedesCorporativo} sedes · usuarios ilimitados)</option>
            </select>
          </div>
