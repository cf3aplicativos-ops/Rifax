...
export default function ConsultaClient({ nonce }: { nonce?: string }) {
  const [state, action, pending] = useActionState(consultarAction, initial);
  const cuenta = state.cuenta;

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <style nonce={nonce}>{"@media print{.no-print{display:none!important}aside{display:none!important}}"}</style>
