...
// dejar el campo vacío conserva el valor actual). Los 4 campos sensibles se
// cifran en reposo con AES-256-GCM (ver crypto-integraciones.ts) — cierra el
// hallazgo de seguridad pendiente desde el 2026-08-11 ("guardados en columnas
// planas... si se requiere cifrado adicional es un cambio aparte").
import "server-only";
import { z } from "zod";
import { prisma } from "@/lib/prisma";
import { auditar } from "@/lib/audit";
import { mensajeError } from "@/lib/errores";
import { encriptar, desencriptar } from "@/lib/crypto-integraciones";
...
export async function obtenerCredencialesWompi(tenantId: bigint): Promise<CredencialesWompi | null> {
  ...
  return {
    sandbox: fila.wompi_sandbox,
    publicKey: fila.wompi_public_key,
    privateKey: desencriptar(fila.wompi_private_key),
    eventsSecret: desencriptar(fila.wompi_events_secret),
  };
}
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.tenant_integraciones.upsert({
        ...
        create: {
          ...
          wompi_private_key: d.wompi_private_key ? encriptar(d.wompi_private_key) : null,
          wompi_events_secret: d.wompi_events_secret ? encriptar(d.wompi_events_secret) : null,
          whatsapp_phone_number_id: d.whatsapp_phone_number_id || null,
          whatsapp_token: d.whatsapp_token ? encriptar(d.whatsapp_token) : null,
          sms_remitente: d.sms_remitente || null,
          sms_api_key: d.sms_api_key ? encriptar(d.sms_api_key) : null,
        },
        update: {
          wompi_sandbox: d.wompi_sandbox,
          wompi_public_key: d.wompi_public_key || null,
          whatsapp_phone_number_id: d.whatsapp_phone_number_id || null,
          sms_remitente: d.sms_remitente || null,
          ...(d.wompi_private_key ? { wompi_private_key: encriptar(d.wompi_private_key) } : {}),
          ...(d.wompi_events_secret ? { wompi_events_secret: encriptar(d.wompi_events_secret) } : {}),
          ...(d.whatsapp_token ? { whatsapp_token: encriptar(d.whatsapp_token) } : {}),
          ...(d.sms_api_key ? { sms_api_key: encriptar(d.sms_api_key) } : {}),
