...
### 2026-08-23 (2) — Cierre de la deuda de seguridad priorizada: CSP, token de un solo uso, cifrado de integraciones, dependencias

El dueño priorizó, tras la auditoría de gobernanza de la entrada anterior, cerrar antes de seguir con features nuevas: los 14 commits sin subir (ya resueltos con el push), 3 errores de lint, `npm audit`, CSP, token de reset de un solo uso, y cifrado de credenciales de integraciones.

- **Lint**: 2 `react/no-unescaped-entities` (`src/app/app/config/page.tsx`) corregidos con `&quot;`. 1 `react-hooks/set-state-in-effect` (`form-asignar.tsx`, ficha de vendedor) corregido reemplazando el `setCargando`/`setSeleccionados` síncronos dentro del efecto por el patrón oficial de React ("ajustar estado durante el render" para el reset, estado derivado de una `loadedKey` para el indicador de carga) — mismo comportamiento observable, sin tocar la lógica de negocio de `asignarTalonarioAction`.
- **`npm audit`**: de 12 vulnerabilidades (11 high, 1 moderate) a 3. `next` actualizado a `16.3.2` (resolvió `next`/`postcss`/`sharp`), `nanoid`/`fast-uri`/`find-my-way`/`valibot` resueltos con `npm audit fix` (bump menor de `prisma`/`@prisma/dev` dentro de 7.x), `nodemailer` actualizado a `9.0.5` (breaking, pero solo usa `createTransport`/`sendMail`, API estable entre mayores; verificado con build + 129 pruebas unitarias + 80 funcionales). Las 3 restantes (`@prisma/config`/`deepmerge-ts`/`prisma`) cuelgan de la CLI de Prisma (`devDependency`, no corre en producción) — mismo patrón ya documentado en el proyecto hermano SISMED; se acepta como riesgo bajo en vez de forzar el downgrade breaking a `prisma@6.12.0`.
- **Hallazgo colateral, no relacionado con lo anterior**: al subir el `target` de `tsconfig.json` (necesario porque `next build` con caché limpio empezó a marcar `BigInt literals are not available when targeting lower than ES2020` en pruebas que ya usaban `1n`/`10n` desde el 2026-08-11 — el error estaba oculto por el caché incremental de TypeScript, nunca antes se había hecho un build con `.next` completamente limpio tras agregar esas pruebas), apareció además un `'solicitud.data' is possibly 'undefined'` real en `test/functional/index.test.ts` (líneas 822 y 887) — corregido con `!` no-nulo, mismo patrón que ya usa el archivo. `target` subido de `ES2017` a `ES2020`.
- **CSP**: no existía ningún `middleware.ts`/`proxy.ts` en el proyecto. Se creó `src/proxy.ts` (Next 16.3.2 exige el nombre `proxy.ts`, `middleware.ts` está deprecado) que solo genera un nonce por petición y la cabecera `Content-Security-Policy` — no toca autenticación ni redirecciones. `script-src` estricto (`'nonce-x' 'strict-dynamic'`, sin `'unsafe-eval'` en producción); `style-src` con `'unsafe-inline'` a propósito, porque hay `style={{...}}` en línea en 6 archivos (`charts.tsx`, login, landing, carrusel, reportes, panel) que un nonce no puede cubrir (el nonce solo aplica a `<style>`/`<script>`, nunca al atributo `style`). Se agregó `nonce={nonce}` a los 11 `<style>` (mezcla de `brandCss()` y estilos de impresión) igualmente, por si en el futuro se decide endurecer `style-src` quitando `unsafe-inline`.
  - **Nonces exigen renderizado dinámico en todas las páginas** (los scripts de hidratación de Next no llevan el nonce si la página se generó estática en build): 3 páginas que eran estáticas (`/consulta`, `/cambiar-password`, `/login/olvide`) se convirtieron en un envoltorio de servidor (`export const dynamic = "force-dynamic"` + lee el nonce con `headers()`) que renderiza un componente cliente nuevo (`*-client.tsx`) con el contenido que ya existía sin cambios — un `page.tsx` con `"use client"` no puede exportar `dynamic` ni llamar `headers()`, de ahí el envoltorio.
  - Verificado en el navegador (servidor local limpio, sin la herramienta de preview que causó falsos 500 en la sesión de SISMED — aquí se usó `npm run dev` directo desde el principio): consola sin errores de CSP en `/login`, `/consulta`, `/login/olvide` y en una landing pública real (`/e/empresa-demo`, confirmando por `getComputedStyle` que `--rifax-accent` sigue aplicando el color de marca con el nonce puesto). `/app` sigue redirigiendo a `/login` sin sesión (307), confirmando que el proxy nuevo no interfiere con `requireUser()`.
- **Token de reset de contraseña de un solo uso**: antes, `solicitarResetAutomatico` cambiaba la contraseña y enviaba la nueva por correo en la misma llamada — cualquiera que conociera el correo de alguien podía forzarle un cambio de contraseña repetidas veces (mitigado solo por rate-limit, nunca eliminado). Ahora es un flujo de dos pasos: **solicitar** (`solicitarResetAutomatico`) valida el correo, crea un token de un solo uso (32 bytes aleatorios, se guarda solo su hash SHA-256, expira en 30 min — tabla nueva `saas.reset_tokens`, migración `prisma/sql/0025_reset_tokens.sql`) y envía un enlace por correo (`mailEnlaceReset`, nueva plantilla); **confirmar** (`confirmarReset`, nueva pantalla `/login/restablecer/[token]`) es lo único que de verdad cambia la contraseña — valida el token, lo marca usado dentro de una transacción (para que dos confirmaciones simultáneas del mismo token no compitan), genera la temporal, revoca sesiones, y ahí sí envía `mailPasswordTemporal` con la nueva clave. `restablecerUsuarioTenant` (el admin resetea a uno de sus usuarios, ya autenticado) no cambió — ese flujo no necesita el token, quien actúa ya probó su identidad al iniciar sesión.
  - Prueba funcional reescrita para el nuevo flujo (`describe("Reset automático de contraseña (token de un solo uso)")`): confirma que solicitar NO toca la contraseña todavía, que confirmar sí, que el mismo token no se puede reusar, y que un token inexistente se rechaza.
  - Verificado en el navegador contra un usuario real de `empresa-demo` (`seelbuga@gmail.com`, encontrado por consulta de solo lectura): se solicitó el reset, se confirmó por el log del servidor que se "envió" (SMTP no configurado en local, queda simulado) el correo con el enlace correcto, y se confirmó por consulta directa que la contraseña **no cambió** — se limpió después el token de prueba creado. Por separado, un token inventado en `/login/restablecer/token-invalido-de-prueba` mostró correctamente "Este enlace ya no es válido."
- **Cifrado de credenciales de integraciones en reposo**: nueva `src/lib/crypto-integraciones.ts` (AES-256-GCM, clave derivada por SHA-256 de la variable de entorno nueva `INTEGRACIONES_ENCRYPTION_KEY` — documentada en el README). `guardarIntegraciones` cifra `wompi_private_key`, `wompi_events_secret`, `whatsapp_token` y `sms_api_key` antes de guardarlos; `obtenerCredencialesWompi` (el único punto que devuelve secretos en claro, para armar el checkout y verificar el checksum del webhook) los descifra. Los campos no sensibles (`wompi_public_key`, `whatsapp_phone_number_id`, `sms_remitente`, `wompi_sandbox`) siguen igual. **No hizo falta migrar datos**: se confirmó por consulta directa que `saas.tenant_integraciones` no tenía todavía ninguna fila con secretos guardados.
  - Pruebas funcionales actualizadas para probar el cifrado real: la fila cruda de la base ya NO coincide con el valor en claro, pero descifrada sí; y `obtenerCredencialesWompi` sigue devolviendo el valor correcto en claro para el flujo de pago.
  - **Pendiente del dueño**: agregar `INTEGRACIONES_ENCRYPTION_KEY` a las variables de entorno de Vercel antes de desplegar — sin ella, guardar o leer una integración falla con un error explícito (no en silencio, no expone nada).
- **Verificación final**: `npm run build` limpio, `npm run lint` 0 errores/0 warnings, `npm test` 129/129, `npm run test:functional` 80/80 (79 anteriores + 1 nueva para el token de reset inválido) sin tenants de prueba residuales.
- **No se pudo desplegar a producción todavía**: este entorno no tiene sesión de Vercel CLI (`npx vercel whoami` → "Logged out" incluso con `XDG_DATA_HOME`/`XDG_CONFIG_HOME` apuntando donde indica esta bitácora; `vercel login` requiere una confirmación interactiva que el dueño tiene que hacer él mismo, y el `VERCEL_OIDC_TOKEN` del `.env` es para identidad de runtime, no sirve para el CLI). El trabajo quedó comiteado y subido a `origin/rifax2` en GitHub; el `vercel deploy --prod --scope rifa7 --yes` y la verificación en vivo contra producción que exige el protocolo de arriba quedan pendientes de que el dueño autentique el CLI (o de que él mismo haga el deploy desde el dashboard).

### 2026-08-26 — Deploy pendiente de la sesión anterior: CSP, token de reset, cifrado de integraciones

El usuario pidió continuar el trabajo en curso. El CLI de Vercel ya estaba autenticado en este entorno (`cf3aplicativos-9248`), así que se pudo cerrar el pendiente que quedó abierto: desplegar a producción los cambios de la entrada anterior (CSP con nonce, token de reset de un solo uso, cifrado de credenciales de integraciones) más el fix posterior de `manifest-src` para Vercel Deployment Protection.

- `npm run build` limpio, `npm test` 129/129. **`npm run test:functional` no pudo correr**: Neon devolvió `Your project has exceeded the data transfer quota` (código Postgres `53000`) al primer query del harness — límite de plan/facturación de la base de datos, no un fallo de la aplicación. El usuario, informado del bloqueo, decidió desplegar igual: el único cambio de código desde el último `80/80` funcional verificado (entrada anterior) era el diff aditivo de 7 líneas de `manifest-src` en `src/proxy.ts` (commit `734aaac`), sin tocar lógica de negocio.
- `vercel deploy --prod --scope rifa7 --yes` exitoso, aliaseado a `https://rifax2.vercel.app`.
- Verificado en vivo en el navegador: login carga sin errores de consola, todos los recursos (`_next/static/...`) responden 200 (nada bloqueado por CSP), y `curl -I` contra producción confirma la cabecera `Content-Security-Policy` con nonce y `manifest-src 'self' https://vercel.com` presentes tal como se codificó.
- **Pendiente**: correr las 80 pruebas funcionales cuando la cuota de Neon se restablezca (o el dueño la amplíe), y que el dueño agregue `INTEGRACIONES_ENCRYPTION_KEY` en Vercel si aún no lo hizo (ver entrada anterior) — sin ella, guardar/leer integraciones falla con error explícito.
