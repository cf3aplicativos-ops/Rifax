...
- **Punto 16**: el vendedor consulta sus propios clientes abonados y qué números juegan (depende de la BD de clientes del punto 15).

**Verificado en vivo**: deploy a producción exitoso (`vercel deploy --prod`) tras cada lote; nav de consulta pública sin "Ingresar (empresa)" confirmado visualmente en el navegador; rutas nuevas (`/app/ventas/abono-otra-sede`, `/api/boletas/disponibles`, `/app/config`) responden 307/401 sin sesión, en local y producción. **No se pudo verificar en vivo con sesión real** (login de admin/cajero/vendedor): las credenciales de memoria de sesiones anteriores ya no son válidas y no se intentó adivinar por fuerza bruta — queda pendiente que el usuario entre y confirme visualmente "Abonados" en un vendedor, la columna "Medio" en ventas, y la pantalla de abono de otra sede.
