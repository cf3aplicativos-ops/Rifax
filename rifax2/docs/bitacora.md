...
**Pendientes (backlog, quedaron sin tocar por ser de varios días cada uno):**
- **Punto 4** (el más grande): landing page personalizada + dominio propio por empresa + pasarela de pagos en línea (compra y abono de boletas) con comisión automática al vendedor dueño de la boleta.
- **Punto 13**: al cerrar una rifa y abrir una nueva, trasladar sedes/vendedores/usuarios (con sus boletas abonadas) previa autorización del admin.
- **Punto 15**: ranking de vendedores al cerrar la rifa, marcarla como finalizada/cerrada, y base de datos de clientes (de todas las sedes) con autocompletar por cédula en una venta nueva.
**Completados en la continuación de la sesión (commits `e7924eb` y `7126c40`, desplegados):**
...
**Quedan 2 puntos grandes en el backlog:**
- **Punto 4** (el más grande): landing page personalizada + dominio propio por empresa + pasarela de pagos en línea (compra y abono de boletas) con comisión automática al vendedor dueño de la boleta. El panel de credenciales de Wompi (punto 7) ya está listo como base, pero falta todo el flujo de checkout/webhook y la gestión de dominios.
- **Punto 13**: al cerrar una rifa (mecanismo ya implementado en el punto 15) y abrir una nueva, trasladar sedes/vendedores/usuarios (con sus boletas abonadas) previa autorización del admin.

**Verificado en vivo**: deploy a producción exitoso (`vercel deploy --prod`) tras cada lote; nav de consulta pública sin "Ingresar (empresa)" confirmado visualmente en el navegador; rutas nuevas (`/app/ventas/abono-otra-sede`, `/api/boletas/disponibles`, `/app/config`, `/api/clientes/buscar`, `/vendedor/clientes`) responden 307/401 sin sesión, en local y producción. **No se pudo verificar en vivo con sesión real** (login de admin/cajero/vendedor): las credenciales de memoria de sesiones anteriores ya no son válidas y no se intentó adivinar por fuerza bruta — queda pendiente que el usuario entre y confirme visualmente "Abonados" en un vendedor, la columna "Medio" en ventas, la pantalla de abono de otra sede, el ranking/cierre de rifa, el autocompletar de cliente y "Mis clientes" del vendedor.
